#include <cloak/transport/mailbox_transport.h>
#include <algorithm>
#include <chrono>
#include <cstring>
...
cloak::core::Result<void> LanMailboxTransport::enqueue(MailboxEnvelope env) {
    ...
    if (queue_bytes_ + msg_bytes > kMaxQueueBytes) {
        return std::unexpected(cloak::core::Error::from(
            cloak::core::ErrorCode::StorageError,
...
cloak::core::Result<std::vector<MailboxEnvelope>> LanMailboxTransport::drain(
...
cloak::core::Result<void> LanMailboxTransport::send(std::span<const std::byte> /*data*/) {
    // In the mailbox replay path the "send" direction is unused; callers
    // never call send() on this transport type.
    return {};
}

cloak::core::Result<std::vector<std::byte>> LanMailboxTransport::receive(
    size_t exact_bytes) {
...
cloak::core::Result<void> LanMailboxTransport::close() {
    open_ = false;
    return {};
}
