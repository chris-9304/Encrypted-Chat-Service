...
public:
    ...
    static cloak::core::Result<GroupSession> create(
        const std::string&              group_name,
        const cloak::identity::Identity&   self);

    // Reconstruct from persisted state (e.g., loaded from SQLite).
    static cloak::core::Result<GroupSession> from_state(
        const cloak::core::GroupId&        group_id,
        const std::string&              group_name,
        cloak::crypto::SecureBuffer<64>    own_sign_sk,
        cloak::core::PublicKey             own_sign_pub,
    ...
    cloak::core::Result<cloak::wire::GroupOpPayload> make_create_op(
        const cloak::core::PublicKey& invitee_signing_pub) const;

    // Build a GroupOp::Invite payload to send to a new member via pairwise DR.
    cloak::core::Result<cloak::wire::GroupOpPayload> make_invite_op(
        const cloak::core::PublicKey& invitee_signing_pub) const;

    // Build a GroupOp::Leave payload to broadcast.
    cloak::wire::GroupOpPayload make_leave_op() const;

    // Process an incoming GroupOp (received via pairwise DR session).
    cloak::core::Result<void> apply_op(const cloak::wire::GroupOpPayload& op);

    // Encrypt a text message for the group.
    cloak::core::Result<cloak::wire::GroupMessagePayload> encrypt(
        const std::string& text) const;

    // Decrypt a group message.  Returns sender display key (signing pub) and text.
    cloak::core::Result<std::pair<cloak::core::PublicKey, std::string>> decrypt(
        const cloak::wire::GroupMessagePayload& msg);

    // ── Accessors ─────────────────────────────────────────────────────────────

    const cloak::core::GroupId&         group_id()    const { return group_id_; }
    const std::string&               group_name()  const { return group_name_; }
    const cloak::core::PublicKey&       own_sign_pub() const { return own_sign_pub_; }
...
private:
    ...
    cloak::core::GroupId            group_id_;
    std::string                  group_name_;

    // Our own sending state.
    mutable cloak::crypto::SecureBuffer<64> own_sign_sk_;
    cloak::core::PublicKey                  own_sign_pub_;
    ...
    MemberState* find_member(const cloak::core::PublicKey& signing_pub);
};
