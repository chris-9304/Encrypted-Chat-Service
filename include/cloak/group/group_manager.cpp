...
Result<GroupId> GroupManager::create_group(
    const std::string& name, const cloak::identity::Identity& self) {
...
Result<void> GroupManager::accept_invite(
    ...
    if (it == groups_.end()) {
        ...
        auto kp = cloak::crypto::Crypto::ed25519_keypair();
        if (!kp) return std::unexpected(kp.error());

        std::array<uint8_t, 32> ck{};
        auto ck_res = cloak::crypto::Crypto::random_bytes(
            std::span<std::byte>(
...
Result<void> GroupManager::apply_op(const cloak::wire::GroupOpPayload& op) {
    std::lock_guard lock(mu_);
    auto it = groups_.find(op.group_id);
    if (it == groups_.end()) {
...
Result<cloak::wire::GroupMessagePayload> GroupManager::send(
...
Result<cloak::wire::GroupOpPayload> GroupManager::leave(const GroupId& gid) {
    std::lock_guard lock(mu_);
    auto it = groups_.find(gid);
    if (it == groups_.end()) {
        return std::unexpected(Error::from(ErrorCode::PeerNotFound,
...
Result<cloak::wire::GroupOpPayload> GroupManager::invite(
...
std::optional<cloak::store::GroupSessionRecord> GroupManager::snapshot(
    ...
    cloak::store::GroupSessionRecord rec;
    ...
    for (const auto& ms : gs.members()) {
        cloak::store::GroupMemberRecord mr;
        mr.group_id = gid;
