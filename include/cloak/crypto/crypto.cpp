...
cloak::core::Result<void> Crypto::initialize() {
    ...
    if (g_sodium_init_failed) {
        return std::unexpected(
            cloak::core::Error::from(cloak::core::ErrorCode::CryptoError,
...
cloak::core::Result<KeyPair> Crypto::kx_keypair() {
    KeyPair kp;
...
cloak::core::Result<Ed25519KeyPair> Crypto::ed25519_keypair() {
    Ed25519KeyPair kp;
...
cloak::core::Result<SharedSecret> Crypto::kx_agree(
...
cloak::core::Result<cloak::core::Signature> Crypto::sign_detached(
    const SecureBuffer<64>& sk, std::span<const std::byte> msg) {

    cloak::core::Signature sig;
...
cloak::core::Result<bool> Crypto::verify_detached(
...
cloak::core::Result<std::vector<std::byte>> Crypto::aead_encrypt(
    ...
    if (nonce.size() != crypto_aead_xchacha20poly1305_ietf_NPUBBYTES) {
        return std::unexpected(
            cloak::core::Error::from(cloak::core::ErrorCode::InvalidArgument,
...
cloak::core::Result<std::vector<std::byte>> Crypto::aead_decrypt(
    const SecureBuffer<32>&    key,
    std::span<const std::byte> nonce,
    std::span<const std::byte> aad,
    ...
    if (ciphertext.size() < crypto_aead_xchacha20poly1305_ietf_ABYTES) {
        return std::unexpected(
            cloak::core::Error::from(cloak::core::ErrorCode::DecryptionFailed,
...
cloak::core::Result<SecureBuffer<32>> Crypto::hkdf_sha256(
...
cloak::core::Result<std::pair<SecureBuffer<32>, SecureBuffer<32>>>
...
cloak::core::Result<SecureBuffer<32>> Crypto::hmac_sha256(
    const SecureBuffer<32>&    key,
    std::span<const std::byte> msg) {
...
cloak::core::Result<SecureBuffer<32>> Crypto::argon2id_derive(
...
cloak::core::Result<std::vector<std::byte>> Crypto::argon2id_salt() {
...
cloak::core::Result<void> Crypto::random_bytes(std::span<std::byte> out) {
    randombytes_buf(out.data(), out.size());
    return {};
}

cloak::core::Result<std::array<uint8_t, 32>> Crypto::blake2b_256(
    std::span<const std::byte> msg) {
