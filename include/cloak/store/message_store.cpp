...
Result<void> MessageStore::save_message(const cloak::wire::Message& message) {
...
cloak::wire::Message read_message_row(sqlite3_stmt* st,
    const std::function<cloak::core::Result<std::string>(std::span<const std::byte>)>& decrypt) {
...
Result<std::vector<cloak::wire::Message>> MessageStore::get_messages_for_peer(
    ...
    std::vector<cloak::wire::Message> msgs;
...
Result<void> MessageStore::load_peers(
    ...
    while (sqlite3_step(st) == SQLITE_ROW) {
        ...
        r.trust = static_cast<cloak::core::TrustStatus>(sqlite3_column_int(st, 5));
