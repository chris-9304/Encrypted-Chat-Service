...
cloak::core::Result<std::vector<std::byte>> encode(const Frame& f);
cloak::core::Result<Frame>                  decode(std::span<const std::byte> bytes);
...
struct HandshakePayload {
    cloak::core::PublicKey x25519_pub;
    cloak::core::PublicKey ed25519_pub;
    cloak::core::Signature sig_over_x25519;
    cloak::core::PublicKey dr_pub;           // DR ephemeral key (Phase 2)
    cloak::core::Signature sig_over_dr;      // authenticates dr_pub
    uint8_t             version{kWireVersion}; // wire protocol version
    std::string         display_name;     // UTF-8, max 64 chars
};

cloak::core::Result<std::vector<std::byte>> encode_handshake(const HandshakePayload& h);
cloak::core::Result<HandshakePayload>        decode_handshake(std::span<const std::byte> bytes);
...
struct AppMessageHeader {
    cloak::core::PublicKey dh_pub; // sender's current DR ratchet key
...
cloak::core::Result<std::vector<std::byte>> encode_app(const AppPayload& p);
cloak::core::Result<AppPayload>              decode_app(std::span<const std::byte> bytes);
...
cloak::core::Result<std::vector<std::byte>> encode_file_chunk(const FileChunkPayload& c);
cloak::core::Result<FileChunkPayload>        decode_file_chunk(std::span<const std::byte> bytes);
...
struct FileMetadata {
    cloak::core::FileId   file_id;
    cloak::crypto::SecureBuffer<32> file_key; // per-file encryption key
...
struct ReceiptPayload {
    ...
    cloak::core::MessageId message_id;
};

cloak::core::Result<std::vector<std::byte>> encode_receipt(const ReceiptPayload& r);
cloak::core::Result<ReceiptPayload>          decode_receipt(std::span<const std::byte> bytes);
...
struct GroupMessagePayload {
    cloak::core::GroupId      group_id;
    cloak::core::PublicKey    sender_sign_pub; // sender's Sender Key signing pub
    uint32_t               message_number{0};
    std::vector<std::byte> ciphertext;
    cloak::core::Signature    signature;
};
...
struct GroupOpPayload {
    ...
    cloak::core::PublicKey member_key;       // signing pub of affected member
    std::array<uint8_t, 32> chain_key{}; // sender's chain key (Invite only)
    uint32_t            chain_counter{0}; // sender's chain counter (Invite only)
};

cloak::core::Result<std::vector<std::byte>> encode_group_message(const GroupMessagePayload& g);
cloak::core::Result<GroupMessagePayload>     decode_group_message(std::span<const std::byte> bytes);

cloak::core::Result<std::vector<std::byte>> encode_group_op(const GroupOpPayload& g);
cloak::core::Result<GroupOpPayload>          decode_group_op(std::span<const std::byte> bytes);
