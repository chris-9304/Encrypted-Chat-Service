...
//   magic  [4] = {0x45, 0x56, 0x52, 0x31}  ("EVR1")
...
//   0x01 = paired as EncryptiV initiator (guest after host responds; host never gets this)
//   0x02 = paired as EncryptiV responder (host when guest arrives)
...
std::string make_invite_code(const ev::core::Endpoint& relay, const RelayRoomId& room);
bool parse_invite_code(const std::string& code,
                       ev::core::Endpoint& out_relay, RelayRoomId& out_room);
...
public:
    // Connect to relay as HOST (EncryptiV responder).
    // Blocks until a guest connects or an error occurs.
    // Returns the transport; caller must use Session::accept().
    static ev::core::Result<std::unique_ptr<Transport>>
        host(const ev::core::Endpoint& relay, const RelayRoomId& room);

    // Connect to relay as GUEST (EncryptiV initiator).
    // Blocks until the relay confirms pairing.
    // Returns the transport; caller must use Session::initiate().
    static ev::core::Result<std::unique_ptr<Transport>>
        join(const ev::core::Endpoint& relay, const RelayRoomId& room);
...
private:
    ...
    static ev::core::Result<std::unique_ptr<Transport>>
        connect_and_pair(const ev::core::Endpoint& relay,
