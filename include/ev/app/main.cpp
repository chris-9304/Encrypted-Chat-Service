...
int main(int argc, char** argv) {
    ...
    if (vm.count("help") || !vm.count("name")) {
        std::cout << "ev-chat 0.4.0 (Phase 4 — Internet Relay)\n\n" << desc << "\n";
    ...
    std::optional<ev::core::Endpoint> relay_ep;
    if (!relay.empty()) {
    ...
    std::cout << "ev-chat 0.4.0  |  name=" << name
              << "  port=" << port;
    if (relay_ep)
        std::cout << "  relay=" << relay_ep->address << ":" << relay_ep->port;
    std::cout << "\n\n";

    g_app = std::make_unique<ev::app::ChatApplication>(
