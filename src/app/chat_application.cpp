...
ChatApplication::ChatApplication(std::string name, uint16_t port,
                                  ...
                                  std::optional<ev::core::Endpoint> relay_endpoint)
...
void ChatApplication::session_recv_func_impl(
    ...
    while (!dead_flag->load() && running_) {
        ...
        case ev::wire::InnerType::Receipt: {
            if (body.size() < 17) break;
            const auto rtype = static_cast<ev::wire::ReceiptType>(body[0]);
            ev::core::MessageId mid;
            std::memcpy(mid.bytes.data(),
                        reinterpret_cast<const std::byte*>(body.data()) + 1, 16);
            if (store_) {
                if (rtype == ev::wire::ReceiptType::Delivered)
        ...
        case ev::wire::InnerType::GroupOp: {
            ...
            if (op_res->op == ev::wire::GroupOpType::Create ||
                ...
                if (op_res->op == ev::wire::GroupOpType::Leave) {
                    std::lock_guard plock(print_mutex_);
        ...
        case ev::wire::InnerType::GroupMessage: {
            auto gmp = ev::wire::decode_group_message(
                std::span<const std::byte>(body));
            if (!gmp) break;
        ...
        case ev::wire::InnerType::DeviceLink: {
        ...
        case ev::wire::InnerType::FileMetadata:
        case ev::wire::InnerType::Typing:
        default:
            ...
            if (store_res) {
                ...
                if (groups_res) {
                    for (auto& rec : *groups_res) {
                        ev::crypto::SecureBuffer<64> own_sk;
                        std::memcpy(own_sk.data(), rec.own_sign_sk.data(), 64);
                        ev::core::PublicKey own_pk;
                        ...
                        std::vector<ev::group::MemberState> members;
                        for (const auto& mr : rec.members) {
                            ev::group::MemberState ms;
                        ...
                        auto gs_res = ev::group::GroupSession::from_state(
                    ...
                    if (device_registry_.is_secondary()) {
                        ...
                        if (cert) {
                            ...
                            std::memcpy(cert_payload.data() + off,
                                        ...
                                        cert->device_name.size());
                            static_cast<void>(
                                s_res->send_inner(ev::wire::InnerType::DeviceLink,
...
void ChatApplication::print_help() const {
    std::cout <<
        ...
        "\nInternet relay commands (Phase 4):\n"
...
void ChatApplication::handle_command(const std::string& line) {
    ...
    // Default: send pairwise text message to active session.
    std::lock_guard lock(session_mutex_);
    if (sessions_.empty() || current_session_idx_ >= sessions_.size() ||
        sessions_[current_session_idx_].dead->load()) {
        std::cout << "[System] No active session. Connect to a peer first.\n";
        return;
    }
    ...
    if (!res) {
        ...
        sessions_[current_session_idx_].dead->store(true); // Bug 1 fixed
        return;
    }
...
void ChatApplication::cmd_peers() {
    ...
    for (size_t i = 0; i < sessions_.size(); ++i) {
        ...
        if (e.dead->load()) { // Bug 2 fixed: was `if (e.dead)`
            ...
            const std::string trust_str =
                rec ? (rec->trust == ev::core::TrustStatus::Verified ? " [verified]" :
                       rec->trust == ev::core::TrustStatus::Changed  ? " [KEY CHANGED]" :
                       "")
                    : "";
...
void ChatApplication::cmd_history() {
    ...
    ev::core::PeerId my_id;
    std::memcpy(my_id.bytes.data(), identity_->signing_public().bytes.data(), 32);

    auto msgs = store_->get_conversation(my_id, peer_id, ev::core::Timestamp{});
    if (!msgs) {
    ...
    for (auto& entry : sessions_) {
        if (!entry.dead->load() && entry.session->is_established()) {
            ...
            auto sr = entry.session->send_inner(
                ev::wire::InnerType::GroupMessage,
...
void ChatApplication::cmd_group_invite(const std::string& args) {
    ...
    if (auto sr = s.send_inner(ev::wire::InnerType::GroupOp,
...
void ChatApplication::cmd_group_leave() {
    ...
    const ev::core::GroupId gid = *current_group_;
...
void ChatApplication::cmd_make_invite() {
    ...
    ev::transport::RelayRoomId room_id{};
    {
        std::array<std::byte, 32 + 16> seed{};
        std::memcpy(seed.data(),
                    identity_->signing_public().bytes.data(), 32);
        static_cast<void>(ev::crypto::Crypto::random_bytes(
            std::span<std::byte>(seed.data() + 32, 16)));
        auto hash = ev::crypto::Crypto::blake2b_256(
    ...
    const std::string code =
        ev::transport::make_invite_code(*relay_endpoint_, room_id);
    ...
    const ev::core::Endpoint relay_ep = *relay_endpoint_;
    std::thread([this, relay_ep, room_id]() {
        auto t_res = ev::transport::RelayTransport::host(relay_ep, room_id);
        if (!t_res) {
            std::lock_guard plock(print_mutex_);
        ...
        // Relay paired us as responder — accept the EncryptiV handshake.
        auto s_res = ev::session::Session::accept(
        ...
        // TOFU check.
        {
            ev::identity::PeerRecord pr;
            pr.signing_public_key = peer_pub;
            pr.display_name       = peer_name;
            pr.fingerprint        = peer_fp;
            auto up = peer_dir_.upsert(pr);
            if (!up && up.error().code == ev::core::ErrorCode::IdentityChanged) {
                std::lock_guard plock(print_mutex_);
                std::cout << "\n[SECURITY] Key change detected for " << peer_name
        ...
        add_session(std::make_unique<ev::session::Session>(std::move(*s_res)));
        {
            std::lock_guard plock(print_mutex_);
...
void ChatApplication::cmd_connect_invite(const std::string& code) {
    ...
    ev::core::Endpoint relay_ep;
    ev::transport::RelayRoomId room_id{};
    if (!ev::transport::parse_invite_code(code, relay_ep, room_id)) {
    ...
    auto t_res = ev::transport::RelayTransport::join(relay_ep, room_id);
    if (!t_res) {
        std::cerr << "[Invite] Relay join failed: " << t_res.error().message << "\n";
        return;
    }

    // Relay paired us as initiator — drive the EncryptiV handshake.
    auto s_res = ev::session::Session::initiate(
        *identity_, my_name_, std::move(*t_res));
    if (!s_res) {
