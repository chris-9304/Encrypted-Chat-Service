...
static constexpr uint8_t kMagic[4]        = {0x45, 0x56, 0x52, 0x31}; // "EVR1"
...
static void handle_host(SharedSocket host_sock, const RoomId& room) {
    ...
    // Tell host it is the EncryptiV responder (Session::accept side).
    boost::system::error_code ec;
...
static void handle_guest(SharedSocket guest_sock, const RoomId& room) {
    ...
    // Tell guest it is the EncryptiV initiator (Session::initiate side).
...
static void handle_client(asio::ip::tcp::socket raw_sock) {
    ...
    if (std::memcmp(hs.data(), kMagic, 4) != 0) {
        send_error(*sock, "bad magic — expected EVR1");
        return;
    }
