...
struct MailboxEnvelope {
    ev::core::PublicKey  recipient_signing_key; // who the message is for
...
public:
    ...
    ev::core::Result<void> enqueue(MailboxEnvelope env);

    // Drain all queued messages for a recipient (called when they reconnect).
    ev::core::Result<std::vector<MailboxEnvelope>> drain(
